    ...
    <div className="px-4 py-6 sm:px-6 lg:px-8">
          ...
          <div onDrop={handleDrop} onDragOver={(e) => e.preventDefault()}>
            {cats.map((cat, idx) => (
              <div
                ...
                style={{
                  borderBottom: idx < cats.length - 1
                    ? "1px solid var(--admin-border)"
                    : "none",
                  background: dragId === cat.id
                    ? "var(--admin-bg)"
                    : "var(--admin-card-bg)",
                }}
              >
      ...
      >
        ...
        <p className="text-xs leading-relaxed" style={{ color: "var(--admin-text-secondary)" }}>
          ...
          delete menu items in it — they will still appear under their original category name on the
          menu. Rename first, then reassign items if needed.
        </p>
      </div>
    </div>
  );
