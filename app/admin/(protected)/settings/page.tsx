...
const ORDER_TYPES = ["Dine In", "Takeaway", "Delivery", "Dine In & Takeaway"] as const;
...
function Field({
  ...
  return (
    <div className="space-y-1.5">
      ...
      {children}
      {hint && !error && <p className="text-xs" style={{ color: "var(--admin-text-muted)" }}>{hint}</p>}
      {error && (
...
export default function SettingsPage() {
  ...
  async function handleSave(e: React.FormEvent) {
    e.preventDefault();
    const errs = validate();
    if (Object.keys(errs).length > 0) { setErrors(errs); toast.error("Fix the errors before saving"); return; }
    setErrors({});
    setSaving(true);
  ...
  return (
    <div className="px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--admin-text-muted)" }}>Configuration</p>
          <h1 className="mt-1 text-2xl font-bold" style={{ color: "var(--admin-text-primary)" }}>Settings</h1>
      ...
      {isDirty && (
        ...
        >
          ...
          <p className="flex-1 text-xs font-medium" style={{ color: "var(--admin-warning)" }}>You have unsaved changes</p>
          <div className="flex items-center gap-2 sm:hidden">
            ...
            <button
              ...
              style={{ background: "var(--admin-accent)" }}
            >
              {saving && <Spinner cls="h-3.5 w-3.5" />}
              Save
            </button>
          </div>
        </div>
      ...
      <form id="settings-form" onSubmit={handleSave} noValidate>
        <div className="grid gap-6 xl:grid-cols-[1fr_300px]">
          ...
          <div className="space-y-6">
            ...
            {/* Brand */}
            <SectionCard id="brand" title="Brand" description="Name and tagline shown to customers on the live menu">
              <div className="grid gap-5 sm:grid-cols-2">
                <Field label="Restaurant Name" error={errors.name} required>
                  <input type="text" value={settings.name} onChange={(e) => set("name", e.target.value)} placeholder="The Spice House" className={inputCls(!!errors.name)} />
                </Field>
                <Field label="Copyright Year">
                  <input type="number" min="2020" max="2100" value={settings.copyright_year} onChange={(e) => set("copyright_year", e.target.value)} className={inputCls(false)} />
                </Field>
              </div>
              <Field label="Tagline" hint="Appears on the hero banner of the customer menu">
                <input type="text" value={settings.tagline} onChange={(e) => set("tagline", e.target.value)} placeholder="Crafted to make you crave more." className={inputCls(false)} />
              </Field>
            </SectionCard>

            {/* Hero Image */}
            <SectionCard id="hero" title="Hero Image" description="Banner shown at the top of the customer menu — landscape, min 1200×600px">
              <Field label="Image URL" error={errors.hero_image} required>
                <input type="url" value={settings.hero_image} onChange={(e) => set("hero_image", e.target.value)} placeholder="https://images.unsplash.com/…" className={inputCls(!!errors.hero_image)} />
              </Field>
              {settings.hero_image && (
                <div className="overflow-hidden rounded-lg" style={{ border: "1px solid var(--admin-border)" }}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={settings.hero_image} alt="Hero preview" className="h-48 w-full object-cover" onError={(e) => ((e.target as HTMLImageElement).style.display = "none")} />
                  <div className="px-4 py-2.5" style={{ borderTop: "1px solid var(--admin-border)" }}>
                    <p className="text-xs" style={{ color: "var(--admin-text-muted)" }}>Recommended: 1200 × 600 px · landscape</p>
                  </div>
                </div>
              )}
            </SectionCard>
            ...
            {/* Contact */}
            <SectionCard id="contact" title="Contact Details" description="Shown in the menu footer and customer receipts">
              <Field label="Address">
                <textarea rows={2} value={settings.address} onChange={(e) => set("address", e.target.value)} placeholder="123 MG Road, New Delhi — 110001" className={cn(inputCls(false), "resize-none")} />
              </Field>
              <div className="grid gap-5 sm:grid-cols-2">
                <Field label="Phone Number">
                  <div className="input-group">
                    ...
                    <input type="tel" value={settings.phone} onChange={(e) => set("phone", e.target.value)} placeholder="+91 98765 43210" />
                  </div>
                </Field>
                <Field label="Email Address" error={errors.email}>
                  <div className={cn("input-group", errors.email && "has-error")}>
                    ...
                    <input type="email" value={settings.email} onChange={(e) => set("email", e.target.value)} placeholder="hello@restaurant.com" />
                  </div>
                </Field>
              </div>
            </SectionCard>
            ...
            {/* Orders & Tax */}
            <SectionCard id="orders" title="Orders & Tax" description="WhatsApp number that receives orders, order mode, and tax rate">
              <Field label="WhatsApp Number" error={errors.whatsapp_number} required hint="Country code + digits only — e.g. 919876543210">
              ...
              <div className="grid gap-5 sm:grid-cols-2">
                <Field label="Order Type">
                  <select value={settings.order_type} onChange={(e) => set("order_type", e.target.value)} className={inputCls(false)}>
                ...
                <Field label="Tax / GST Rate" error={errors.tax_rate} required>
                  <div className="relative">
                    <input
                      ...
                      className={cn(inputCls(!!errors.tax_rate), "pr-9")}
                    />
            ...
            <div className="sticky top-[76px] space-y-4">
              ...
              <div className="overflow-hidden rounded-xl" style={{ background: "var(--admin-card-bg)", border: "1px solid var(--admin-border)" }}>
                <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: "1px solid var(--admin-border)" }}>
                  <p className="text-xs font-bold uppercase tracking-widest" style={{ color: "var(--admin-text-muted)" }}>Live Preview</p>
                ...
                <div className="p-4" style={{ background: "var(--admin-bg)" }}>
                  {/* Navbar mockup */}
                  <div className="flex items-center justify-between rounded-lg px-3 py-2.5" style={{ background: "var(--admin-card-bg)", border: "1px solid var(--admin-border)" }}>
                    ...
                    <div className="ml-2 shrink-0 rounded-md px-2 py-1 text-center" style={{ background: "var(--admin-bg)", border: "1px solid var(--admin-border)" }}>
                  ...
                  <div className="relative mt-2.5 h-24 overflow-hidden rounded-lg bg-neutral-900">
                    {settings.hero_image && (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={settings.hero_image} alt="" className="h-full w-full object-cover opacity-60" onError={(e) => ((e.target as HTMLImageElement).style.display = "none")} />
                    )}
                  ...
                  {/* Footer mockup */}
                  <div className="mt-2.5 rounded-lg px-3 py-2.5 text-center" style={{ background: "var(--admin-card-bg)", border: "1px solid var(--admin-border)" }}>
                    ...
                    {settings.phone && <p className="mt-0.5 text-[9px]" style={{ color: "var(--admin-text-muted)" }}>{settings.phone}</p>}
                  </div>
                </div>

                {/* Stats row */}
                <div className="grid grid-cols-2" style={{ borderTop: "1px solid var(--admin-border)" }}>
                  <div className="px-5 py-4 text-center" style={{ borderRight: "1px solid var(--admin-border)" }}>
                    ...
                    <p className="mt-1 text-lg font-bold" style={{ color: "var(--admin-text-primary)" }}>{settings.tax_rate || "0"}%</p>
                  </div>
                  <div className="px-5 py-4 text-center">
                    <p className="text-[10px] font-semibold uppercase tracking-widest" style={{ color: "var(--admin-text-muted)" }}>Mode</p>
                    <p className="mt-1 truncate text-xs font-bold" style={{ color: "var(--admin-text-primary)" }}>{settings.order_type}</p>
                  </div>
                </div>
              ...
              {/* Jump-to nav */}
              <div className="rounded-xl p-4" style={{ background: "var(--admin-card-bg)", border: "1px solid var(--admin-border)" }}>
                <p className="mb-3 text-[11px] font-bold uppercase tracking-widest" style={{ color: "var(--admin-text-muted)" }}>Jump to</p>
